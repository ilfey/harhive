...
export const stringifyNetworkData = (
	value: NetworkData,
): string | string[] | null => {
...
export const formatUrl = (
	...
	const queryString =
		typeof queryRecord === "string"
			? queryRecord
			: recordToUrlSearchParams(queryRecord).toString();
...
export const mergeRecords = (
	...records: (NetworkRecord | undefined | null)[]
): NetworkRecord => {
...
export const mergeQueryStrings = (
	...
	for (const item of queryStrings) {
		...
		const current =
			typeof item !== "string"
				? recordToUrlSearchParams(item).toString()
				: item;
