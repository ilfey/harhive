import {
	fetchFx,
	HttpError,
	httpError,
	NetworkError,
	networkError,
} from "@farfetched/core";
import { createEffect } from "effector";
import {
	formatHeaders,
	formatUrl,
	mergeQueryStrings,
	mergeRecords,
	type NetworkRecord,
} from "./networkUtils";

type RequestBody = Blob | BufferSource | FormData | string;
type HttpMethod = "GET" | "POST" | "PUT" | "PATCH" | "DELETE" | "OPTIONS";

// type ReplaceUndefinedToNull<T extends object> = { [K in keyof T]-?: T extends Record<K, T[K]> ? T[K] : Exclude<T[K], undefined> | null }

type FactoryParams = {
	baseUrl?: string;
	headers?: NetworkRecord;
	query?: NetworkRecord | string;
};

export type StaticParams = {
	method: HttpMethod;
	url: string;
	credentials?: RequestCredentials;
	abortController?: AbortController;
	bodyMapper?: (body: unknown) => RequestBody;
};

export type DynamicParams = {
	query?: NetworkRecord | string;
	headers?: NetworkRecord;
	body?: unknown;
};

export type RequestParams<Params> = (
	params: Params,
) => StaticParams & DynamicParams;

export const createRequestBuilder =
	(factoryParams: FactoryParams) =>
	<Params, Data>(requestParams: RequestParams<Params>) =>
		createEffect<
			Params,
			// ReplaceUndefinedToNull<StaticParams> & ReplaceUndefinedToNull<DynamicParams>,
			Data,
			NetworkError | HttpError
		>(async (params) => {
			const {
				method,
				url,
				credentials,
				abortController,
				bodyMapper,

				query,
				headers,
				body,
			} = requestParams(params);

			const mapper = bodyMapper ?? ((body: unknown) => JSON.stringify(body));

			const mergedHeaders = mergeRecords(factoryParams.headers, headers ?? {});

			const request = new Request(
				formatUrl(
					factoryParams.baseUrl ?? null,
					url,
					mergeQueryStrings(factoryParams.query, query ?? {}),
				),
				{
					method: method,
					headers: formatHeaders(mergedHeaders),
					credentials: credentials,
					body: mapper(body),
					signal: abortController?.signal,
				},
			);

			const response = await requestFx(request);

			return await response.json();
		});

const requestFx = createEffect<Request, Response, NetworkError | HttpError>(
	async (request) => {
		const response = await fetchFx(request).catch((cause) => {
			throw networkError({
				reason: cause?.message ?? null,
				cause,
			});
		});

		if (response.status >= 500 || response.status === 401) {
			const res = await response.text().catch(() => null);

			throw httpError({
				status: response.status,
				statusText: response.statusText,
				response: res,
			});
		}

		return response;
	},
);
